...
from model.musicD import Discriminator
from model.musicG import Generator
import numpy as np
...
dataset = Songs(dataPath=opt.dataPath,length=1000)
...
real_A = torch.FloatTensor(opt.batchSize, input_nc, 16384*30)
real_B = torch.FloatTensor(opt.batchSize, input_nc, 16384*30)
...
for epoch in range(1,opt.niter+1):
    ...
    for i, image in enumerate(train_loader):
        ...
        #real_A = Variable(imgA)
        #real_B = Variable(imgB)
        #if(opt.cuda):
        #    real_A = real_A.cuda()
        #    real_B = real_B.cuda()
